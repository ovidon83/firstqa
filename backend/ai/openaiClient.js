...
async function generateQAInsights({ repo, pr_number, title, body, diff, newCommits, fileContents = {}, selectorHints = [] }) {
  try {
    ...
    // Dynamic diff budget based on model context window (TPM cap removed — user manages PR size)
...
- recommendations: array of max 5 items. Each item must be a REAL change needed for the ticket to be clear and dev-ready: e.g. change of copy/wording, change or addition of acceptance criteria, new mandatory AC, or a concrete gap to fix. Only things genuinely MISSING or INCORRECT in this ticket. Do NOT repeat, rephrase, or summarize anything already stated in the ticket description, acceptance criteria, or comments. If a requirement is already clearly defined in the ticket, do not include it as a recommendation. If the ticket is well-written with no gaps, return an empty array. No generic best practices. Ready-to-paste, full sentences. Will be shown as a checklist.
